...
module.exports = class StringHelper {
    static formatObjectsFixedWidth(objects, columns, distance, options = {}) {
        const { numbering = false } = options;

        const maxWidths = columns.reduce((result, column) => Object.assign(result, { [column]: Math.max(...objects.map(object => ('' + object[column]).length)) }), {});
